...
// define a ring buffer of 16 units with 8 bit data
...
// extract integer part from the division factor N
#define UART_DIVISOR_INT_16BIT ((uint16_t)UART_DIVISOR)
//since the registers of the msp430 are 8 bit we need 
// to divide the above prescaler  into two parts
#define UART_DIVISOR_INT_LOW_BYTE (UART_DIVISOR_INT_16BIT & 0xFF)
#define UART_DIVISOR_INT_HIGH_BYTE (UART_DIVISOR_INT_16BIT >> 8)
// extract fraction part from the division factor 
// by subtarcting off the integer part
#define UART_DIVISOR_FRACTIONAL (UART_DIVISOR - UART_DIVISOR_INT_16BIT)
// setting the modultor round(fraction part* 8)
...
// diasble any interrupt incase of an assert call
...
// for assert we used polling to transmit characters
